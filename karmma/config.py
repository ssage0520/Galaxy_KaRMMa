...
import jax
import healpy as hp
from karmma.structs import (McmcConfig, AnalysisConfig, IoConfig,
                             XlmParams, ThetaParams, KarmmaPosition)
...
def _load_theta(path, group):
    with h5.File(path, 'r') as f:
        return ThetaParams(**{field: f[f'{group}/{field}'][:] for field in ThetaParams._fields})
...
class KarmmaConfig:
    ...
    def _set_mcmc(self, cfg):
        ...
        else: step_size = float(step_size)
        ...
        else: target_acceptance = float(target_acceptance)
