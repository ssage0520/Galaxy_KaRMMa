...
def get_corrfunc(field_maps, mask, min_sep=None, max_sep=300., nbins=15, npatch=50):
...
def get_field_bins(field, mask, n_bins=46, n_sigma_linear=4):
    ...
    for i in range(n_zbins):
        ...
        linear_bins_i = np.linspace(field_min - linear_width, n_sigma_linear * std_i, n_bins)
...
def setup_pseudo_cls(mask, n_ell_bins=17):
    ...
    ell_edges = np.ceil(np.logspace(np.log10(3), np.log10(lmax), n_ell_bins)).astype(int)
...
def get_pseudo_cls(field_maps, mask, nmt_ell_bins, workspace):
    ...
    fields = [nmt.NmtField(mask.astype(float), [field_maps[i]], spin=0, lmax=lmax)
              for i in range(n_zbins)]

    cls = np.zeros((n_zbins, n_zbins, n_ell))
    for i in range(n_zbins):
        for j in range(i+1):
            cl_ij = nmt.compute_full_master(fields[i], fields[j], nmt_ell_bins, workspace=workspace)
            cls[i,j] = cl_ij[0]
            cls[j,i] = cl_ij[0]
