...
from scipy.special import roots_legendre, legendre_p_all
from karmma.transforms import alm2map, map2alm
from karmma.structs import XlmParams, ThetaParams, KarmmaPosition, NUTSInfo
import blackjax
...
class KarmmaSampler:
    def __init__(
        ...
        self.Ng_obs = np.round((dg_obs[:, self.mask] + 1.0) * self.N_bar[:, None]).astype(np.int32)
    ...
    def _compute_CL_G_binpair(self, i, j, ell_array, P_ell, w):
        ...
        xi_G = np.log(1 + xi_NG / (self.beta[i]*self.beta[j])) / (self.alpha[i]*self.alpha[j])
    ...
    def compute_CL_G(self, order=2):
        ...
        for i in range(self.Nbins):
            for j in range(i+1):
                self.CL_G[i,j,:] = self._compute_CL_G_binpair(i, j, ell_array, P_ell, w)
                if i!=j:
    ...
    def x2deff(self, xlm: XlmParams, theta: ThetaParams):
        ...
        dm    = self.beta[:, None] * (jnp.exp(self.alpha[:, None]*ys - 0.5*self.alpha[:, None]**2) - 1)
        dm_lm = map2alm(dm, self.lmax)
        b_ell = jnp.exp(-0.5 * self.ell*(self.ell+1) * (jnp.exp(theta.log_R[:, None]) * self.pixel_size)**2)
        filt  = (1.0 + theta.c[:, None]*b_ell) * (self.pixwin[self.ell] if self.pixwin is not None else 1.0)
    ...
    def make_random_xlm(self, key):
        ...
        return XlmParams(
            real=jax.random.normal(rk, shape=(self.Nbins, len(self._real_idx)), dtype=jnp.float64),
            imag=jax.random.normal(ik, shape=(self.Nbins, len(self._imag_idx)), dtype=jnp.float64),
        )
    ...
    def log_prob(self, params: KarmmaPosition):
        ...
        if self.infer_theta:
            ...
            log_prior_theta = (
                ...
                - 2.0 * (1.0 + _INVGAMMA_ALPHA_R) * jnp.sum(theta.log_R)   # InvGamma log-prior
                - _INVGAMMA_BETA_R * jnp.sum(jnp.exp(-2.0 * theta.log_R))  # InvGamma log-prior
            )

        return log_prior_real + log_prior_imag + log_jacobian_theta + log_prior_theta + log_lik

    def sample(self, key, num_warmup, num_samples,
            initial_position: KarmmaPosition,
            step_size=0.05, target_acceptance_rate=0.65):
        ...
        print()
        (wstate, parameters), winfo = warmup.run(warmup_key, initial_position,
        ...
        print(f'Mean integration steps (warmup): {warmup_steps.mean():.1f}  |  last 20: {mean_steps_end:.1f}')
        print(f'Mean acceptance rate (warmup): {jnp.mean(winfo.info.acceptance_rate):.4f}')
        print(f'Number of divergences (warmup): {jnp.sum(winfo.info.is_divergent)}')
        print(f'Estimated sampling time: ~{timedelta(seconds=int(est_sampling_time))}  ({num_samples} samples × ~{time_per_sample:.1f}s/sample)')
        ...
        _, (states, infos) = blackjax.util.run_inference_algorithm(
            ...
            progress_bar=True,
            transform=lambda state, info: (state.position, NUTSInfo(
        ...
        print(f'Mean integration steps: {np.array(infos.num_integration_steps).mean():.1f}')
