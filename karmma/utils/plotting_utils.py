...
def plot_map(dm_map, mask, minmax, cmap='viridis', cb_label=r'$\delta_m$', 
             title=None, ax=None):
...
def plot_dm_comparison(dm_true, dm_mean, mask, n_samples=None, cmap='viridis'):
    ...
    for i in range(nbins):
        ...
        plot_map(dm_true[i], mask, minmax=minmax, cmap=cmap,
                 cb_label=r'$\delta_m$',
                 title=f'Bin {i+1} — True $\\delta_m$',
                 ax=axes[i, 0])
        plot_map(dm_mean[i], mask, minmax=minmax, cmap=cmap,
                 cb_label=r'$\delta_m$',
...
def plot_corr(corr_samples, corr_true, bin_centres, ylim=None, interval=68):
    ...
    for i in range(nbins):
        for j in range(nbins):
            ...
            l3, = ax.semilogx(bin_centres, np.ones_like(bin_centres), 'k--', linewidth=1.0)

            ax.set_ylim(ylim)
            ax.set_xlim(bin_centres[0], bin_centres[-1])
            ax.text(0.05, 0.85, rf'$\xi_{{{i+1}{j+1}}}$',
    ...
    axes[0, 1].legend(handles=[l1, l2, l3],
                      labels=['Sample mean', f'Samples ({interval}th percentile)', 'Truth'],
                      loc='center', fontsize=11, framealpha=0.9)
...
def plot_pseudo_cl(cl_samples, cl_true, eff_ell, nside, ylim=None, interval=68):
    ...
    for i in range(nbins):
        for j in range(nbins):
            ...
            ax.text(0.95, 0.85, rf'$C^{{{i+1}{j+1}}}_{{\ell}}$',
                    ha='right', transform=ax.transAxes, fontsize=11)
...
def plot_1pt_linear(pdf_linear_samples, pdf_linear_true, linear_bins, interval=68):
    ...
    fig, axes = plt.subplots(1, nbins + 1, figsize=(4*nbins + 2, 4),
    ...
    for i in range(nbins):
        ...
        ax.fill_between(np.repeat(edges, 2)[1:-1],
    ...
    legend_ax.axis('off')
    legend_ax.legend(
        handles=[Line2D([0], [0], color='b', linewidth=1.5),
