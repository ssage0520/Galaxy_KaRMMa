...
if recompute_corr:
    corr_true, corr_errors_true, corr_bin_centres, corr_bin_edges = get_corrfunc(
        dm_true, mask
    )
...
with h5.File(os.path.join(io_dir, "samples.h5"), "r") as f:
    for i in trange(n_samples):
        ...
        if recompute_1pt:
            pdf_linear_samples[i], pdf_log_samples[i] = get_1ptfunc(
                dm_i, mask, linear_bins, log_bins
            )
